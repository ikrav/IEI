...
nbins_dummy = 100
plotspace = TH2F("plotspace", "Fit example for graph with errors", nbins_dummy, xmin, xmax, 
...
# Next, we will create a fit function
func = TF1("func","[0] + [1] * x + [2] * x*x", xmin, xmax)
func.SetParameters(1,2,3) # any non-crazy non-zero values

# Perform a fit
graph.Fit("func")

# Set up a text message
first_line  = "A = {:.3f} #pm {:.3f}".format( func.GetParameter(0), func.GetParError(0) )
second_line = "B = {:.3f} #pm {:.3f}".format( func.GetParameter(1), func.GetParError(1) )
third_line  = "C = {:.3f} #pm {:.3f}".format( func.GetParameter(2), func.GetParError(2) )

comments = TLatex()
comments.SetNDC(ROOT.kTRUE) # makes coordinates below go from 0 to 1 for X and Y
comments.DrawLatex(0.15, 0.8, first_line)
comments.DrawLatex(0.15, 0.7, second_line)
comments.DrawLatex(0.15, 0.6, third_line)
